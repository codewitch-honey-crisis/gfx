#ifndef HTCW_GFX_DRAW_AA_POLYLINE_HPP
#define HTCW_GFX_DRAW_AA_POLYLINE_HPP
...
// Anti-aliased, variable-width polyline with end caps and line joins.
//
// The stroke is composited in a single pass per scanline as a DISTANCE FIELD:
// each primitive reports a signed distance (in the same 16.16 "dist16 = hw + SDF"
// form the line routine uses), the per-pixel MINIMUM is kept, and that single
// combined field is converted to coverage once at the end. Because the union of
// shapes has distance field min(members), the anti-aliased boundary is computed
// on the true outer edge of the whole stroke -- overlapping pieces neither
// double-darken nor leave the half-lit creases you get from max-ing coverages of
// abutting pieces.
//
// Joins:
//   round  : the segment ends meeting at an interior vertex are round-capped; the
//            min-distance of the two capsules is exactly the round join. No extra
//            geometry, no side/winding decision.
...
// outer corner (spike/chord) is unaffected. Even if the inward extension makes
// the fill slightly non-convex on the inner side, the segments backstop it via
// the min, so it can never carve a gap.
//
...
class xdraw_aa_polyline {
    ...
    // (Verified against the (10,0)->(10,25)->(50,25)... test path with +1.)
    ...
    // pixel is beyond this primitive's reach.
    ...
    // signed distance (dist16) of a filled dot/square centered at (cxp,cyp).
    static int32_t point_dist16(int32_t px, int32_t py, int32_t cxp, int32_t cyp,
                                line_cap cap, int32_t hw16, int32_t hw_q8, int32_t cap_cut2) {
        const int32_t fx = px - cxp, fy = py - cyp;
        if (cap == line_cap::square) {
            const int32_t axv = fx < 0 ? -fx : fx;
            const int32_t ayv = fy < 0 ? -fy : fy;
            const int32_t m = axv > ayv ? axv : ayv;
            return hw16 + ((m << 8) - hw_q8) * 256;
        }
    ...
    // Build the convex-ish join fill at interior vertex V (prev P, next N) in
    // V-local Q8 coordinates. Vertices run: inner anchor in the incoming segment,
    // the incoming outer corner, [miter apex], the outgoing outer corner, inner
    ...
    static int32_t poly_dist16(int32_t plx_q8, int32_t ply_q8, const poly_edge* edges,
        ...
        // maxd is the signed distance to the fill boundary in Q8 (>0 outside).
        // dist16 = hw + SDF, and here the fill already encodes hw, so SDF = maxd - 0
        // relative to a zero-width shape: express as hw + maxd so band16 - dist16
        // == 0.5 - maxd, matching the segment convention.
    ...
    static gfx_result aa_polyline_impl(Destination& destination, const spath16& path,
        ...
        if (cap == line_cap::square) {
            int p2 = (int)(width >> 1) + (int)(width >> 2) + 2;
            if (p2 > pad) pad = p2;
        }
        if (use_miter) {
        ...
        for (int py = miny; py <= maxy; ++py) {
            ...
            if (np == 1) {
                if (cap != line_cap::butt) {
                    const spoint16 p0 = path[0];
                    const int32_t cxp = p0.x, cyp = p0.y;
                    int xs = cxp - pad, xe = cxp + pad;
                    if (xs < minx) xs = minx;
                    if (xe > maxx) xe = maxx;
                ...
                for (size_t sidx = 0; sidx < nseg; ++sidx) {
                    ...
                    const spoint16 pbt = path[sidx + 1];
                    ...
                    const line_cap capA = (sidx == 0)        ? cap : join_end_cap;
                    const line_cap capB = (sidx == nseg - 1) ? cap : join_end_cap;
                ...
                if (poly_join && np >= 3) {
                    for (size_t k = 1; k + 1 < np; ++k) {
                        const spoint16 pv = path[k];
                        const spoint16 pp = path[k - 1];
                        const spoint16 pn = path[k + 1];
...
public:
    // draws an anti-aliased polyline of the given width (>= 0), color, end cap and join.
    ...
    template <typename Destination, typename PixelType>
    inline static gfx_result aa_polyline(Destination& destination, const spath16& path,
        ...
        return aa_polyline_impl(destination, path, color, width, cap, join, miter_limit, cache, clip);
    }
};
}
}
