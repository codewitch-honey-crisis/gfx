...
NSVGrasterizer* nsvgCreateRasterizer(void*(allocator)(size_t)=::malloc,void*(reallocator)(void*,size_t)=::realloc,void(deallocator)(void*)=::free);
...
NSVGrasterizer* nsvgCreateRasterizer(void*(allocator)(size_t),void*(reallocator)(void*,size_t),void(deallocator)(void*))
{
...
static NSVGmemPage* nsvg__nextPage(NSVGrasterizer* r, NSVGmemPage* cur)
{
	...
	// Alloc new page
	newp = (NSVGmemPage*)r->allocator(sizeof(NSVGmemPage));
...
static void nsvg__addPathPoint(NSVGrasterizer* r, float x, float y, int flags)
{
	...
	if (r->npoints+1 > r->cpoints) {
		...
		r->points = (NSVGpoint*)r->reallocator(r->points, sizeof(NSVGpoint) * r->cpoints);
...
static void nsvg__duplicatePoints(NSVGrasterizer* r)
{
	if (r->npoints > r->cpoints2) {
		...
		r->points2 = (NSVGpoint*)r->reallocator(r->points2, sizeof(NSVGpoint) * r->cpoints2);
...
static void nsvg__addEdge(NSVGrasterizer* r, float x0, float y0, float x1, float y1)
{
	...
	if (r->nedges+1 > r->cedges) {
		...
		r->edges = (NSVGedge*)r->reallocator(r->edges, sizeof(NSVGedge) * r->cedges);
...
static void nsvg__scanlineSolid(unsigned char* dst,NSVGrasterizerReadCallback readCallback, void* readCallbackState,NSVGrasterizerWriteCallback writeCallback, void* writeCallbackState, int count, unsigned char* cover, int x, int y,
...
//					NSVG__assert(z->valid);
		...
		if (xmin <= xmax) {
			nsvg__scanlineSolid(r->bitmap!=nullptr?&r->bitmap[y * r->stride] + xmin*4:nullptr,r->readCallback,r->readCallbackState,r->writeCallback,r->writeCallbackState, xmax-xmin+1, &r->scanline[xmin], xmin, y, tx,ty, scale, cache);
		}
	}
